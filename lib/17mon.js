...
exp.loadDB = function(filepath) {
    try {
        var newDataBuffer = loadBinaryData(filepath)
        var checkIP = '8.8.8.8'
        var result = IpFind(newDataBuffer, checkIP)
        console.log('check ip %s %j', checkIP, result)
        if (!!result && result[0] === 'GOOGLE') {
        ...
        console.error('load db file %s failed err=%s', filepath, e.stack)
    }
}

exp.lookup = function(ip) {
    var result = IpFind(dataBuffer, ip)
    return {
        country: result[0],
        province: result[1],
        city: result[2],
        orgnization: result[3],
        isp: result[4]
    }
}
...
var IpFind = function(dataBuffer, ip){
